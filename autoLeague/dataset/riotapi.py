...
'''Dataset generator for League of Legends replays by tier.'''
class RiotAPI(object):

    '''
        api_key : Riot API key
        region  : Platform routing value (e.g., "JP1", "KR", "NA1")
    '''
    # Region to routing mapping (for match-v5 API)
    REGION_TO_ROUTING = {
        "JP1": "asia", "KR": "asia",
    ...
    def __init__(self, api_key, region="JP1"):
        self.api_key = api_key
        self.region = region.upper()
        self.routing = self.REGION_TO_ROUTING.get(self.region, "asia")
        self.region_prefix = f"{self.region}_"  # e.g., "JP1_" or "KR_"


    def getPuuid(self, summonerId):
        try:
            url = f'https://{self.region.lower()}.api.riotgames.com/lol/summoner/v4/summoners/{summonerId}?api_key={self.api_key}'
    ...
    def getSummonerId(self, puuid):
        try:
            url = f'https://{self.region.lower()}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/{puuid}?api_key={self.api_key}'
            summonerId = requests.get(url).json()['id']
            return summonerId
        except (requests.RequestException, KeyError, ValueError):
            return None
    ...
    def getMatchData(self, matchId):
        data = requests.get(f"https://{self.routing}.api.riotgames.com/lol/match/v5/matches/{matchId}/timeline?api_key={self.api_key}").json()
    ...
    #     # Directory containing replay files (example path)
    ...
    #                 else:
    #                     print(f"Match {match_id} does not have 10 champions.")
    ...
    def save_champnames_from_matches_with_rofl(
        ...
        csv_name: str,
        replay_dir: str,
        max_workers: int = 1,          # Number of concurrent threads (1 recommended for rate limit handling)
    ):
        """When only replay(.rofl) files exist:
        Extract matchid from filename → Query 10 champion names via Riot API → Save to CSV
        """
        # ── Internal function (Rate limit handling) ─────────────────────────────────
        def fetch_champnames(match_id: str, max_retries: int = 5) -> list[str]:
            # Replace '-' with '_' for API call
            matchid_api = match_id.replace("-", "_")
            url = (
                f"https://{self.routing}.api.riotgames.com/lol/match/v5/matches/{matchid_api}"
            ...
            for attempt in range(max_retries):
                ...
                # If 'info' key doesn't exist (e.g., 404)
                if "info" not in data:
                    raise KeyError(f"status={resp.status_code}, response={data}")

                return [p["championName"] for p in data["info"]["participants"]]

            raise Exception(f"Max retries exceeded for {matchid_api}")
        ...
        # ── Progress bar setup (tqdm) ─────────────────────────────────────────
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            futures = {
                pool.submit(fetch_champnames, mid): mid for mid in match_ids
            }

            # Open CSV once and write from main thread
            with open(csv_file, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)

                for fut in tqdm(as_completed(futures), total=len(match_ids), desc="Fetching match data"):
                    mid = futures[fut]
                    try:
                        champs = fut.result()
                        if len(champs) == 10:
                            ...
                            print(f"⚠️  {mid}: champion count {len(champs)}")
                    except Exception as e:
    ...
    #     # Write header only if it doesn't exist
    #     if not os.path.exists(csv_file):
    #         with open(csv_file, mode='w', newline='', encoding='utf-8') as f:
    #             writer = csv.writer(f)
    #             writer.writerow(['match_id'] + [f'champion_{i+1}' for i in range(10)])

    #     # Write one by one in append mode
    ...
    def save_champnames_from_matches_without_rofl(
        ...
        output_csv_path: str,
        max_workers: int = 1,          # Number of concurrent threads (1 recommended for rate limit handling)
        request_interval: float = 1.3, # Seconds between requests (100 req/120s = 1.2s, use 1.3s for safety)
    ):
        """When skipping download_replays.ipynb step:
        Read matchid from CSV and query 10 champions via Riot API, then save.

        Args:
            input_csv_path: Input CSV path containing matchid column
            output_csv_path: Output CSV path for champion data
            max_workers: Number of concurrent threads (1 recommended)
            request_interval: Seconds between API requests (default 1.3s for rate limit)
        """
        # ── Internal function (Rate limit handling) ─────────────────────────────────
        def get_champnames_per_match(matchid: str, max_retries: int = 5) -> list[str]:
            # Rate limit: 100 requests per 120 seconds → wait before each request
            time.sleep(request_interval)

            url = (
                f"https://{self.routing}.api.riotgames.com/lol/match/v5/matches/{matchid}"
                f"?api_key={self.api_key}"
            )
            for attempt in range(max_retries):
                resp = requests.get(url, timeout=10)
        ...
        # ── Result CSV header (first time only) ─────────────────────────────────
        if not os.path.exists(output_csv_path):
            with open(output_csv_path, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(["match_id"] + [f"champion_{i+1}" for i in range(10)])

        # ── Parallel collection + progress bar (tqdm) ─────────────────────────────────
        with ThreadPoolExecutor(max_workers=max_workers) as pool:
            futures = {pool.submit(get_champnames_per_match, mid): mid for mid in matchids}

            # Open CSV once in append mode and write sequentially
            with open(output_csv_path, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)

                for fut in tqdm(as_completed(futures), total=len(matchids), desc="Fetching match data"):
                    ...
                    try:
                        ...
                        if len(champs) == 10:
                            ...
                            print(f"⚠️  {match_id}: champion count {len(champs)}")
                    except Exception as e:
    ...
    def save_top_matches_by_score(
        ...
        with open(avg_score_csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                m_id = row["match_id"]
                if m_id.startswith(self.region_prefix):
                    m_id = m_id.replace(self.region_prefix, "")
    ...
    def find_disjoint_matches_combinations(target_matches, max_solutions):
        # target_matches = 7  # Example: select 7 matches (original was 15, changed to 7 for testing)
        # max_solutions = 3   # Set to find up to 3 combinations
    ...
    def find_disjoint_matches_combinations_with_initial(
        ...
        def find_disjoint_matches_additional(matches, target, start_index,
            ...
            (matches is a list of (match_id, champion_set))
            target is 'the total number of match_ids we want in the final result'
            Starts searching from start_index,
            if no overlapping champions, selects match_id (adds to selected) and proceeds to next step.
            """
        ...
        # (if target_matches < current_len, can't reduce further so return as is)
    ...
    def find_matches_combinations_with_initial_allow_duplicate_2(
        ...
        def get_champnames_per_match(matchid):
            url = f'https://{self.routing}.api.riotgames.com/lol/match/v5/matches/{matchid}?api_key={self.api_key}'
    ...
    def find_disjoint_matches_combinations_with_initial_ver2(
            ...
            max_duplicates: int = 0              # ← New: allow up to n duplicates
    ):
    ...
    def find_disjoint_matches_combinations_with_initial_ver3(
        ...
        - initial_matches: List of already selected match_ids
        - max_solutions  : Maximum number of solutions to return
        - verbose=True   : Print progress logs and statistics
        """
    ...
    def find_best_coverage_with_additional_matches(
        ...
        def champs_api(mid):
            if mid in _cache:
                return _cache[mid]
            url = f"https://{self.routing}.api.riotgames.com/lol/match/v5/matches/{mid}?api_key={self.api_key}"
            data = requests.get(url, timeout=3).json()
            names = {p["championName"] for p in data["info"]["participants"]}
            _cache[mid] = names
            return names
    ...
    def save_summoner_leagueinfo_of_replays(
        ...
        def process_one_match(rofl_name: str):
            ...
            # 1) match → 10 puuids
            url_match = (f"https://{self.routing}.api.riotgames.com/lol/match/v5/matches/"
            ...
            players = []
            for puuid in puuids:
                url_rank = (f"https://{self.region.lower()}.api.riotgames.com/lol/league/v4/entries/by-puuid/"
    ...
    def save_summoner_leagueinfo_from_csv(
        ...
        - CSV must have 'match_id' column.
        - Uses simple print statements instead of rich.Progress for Jupyter notebook compatibility.
        """
        ...
        match_ids = [mid for mid in df["match_id"].dropna().unique()
                    if str(mid).startswith(self.region_prefix)]
    ...
    def save_match_avg_score_from_json(
        ...
        Args:
            match_info_dir (str)  : Directory containing KR_xxx.json files
            output_csv_path (str) : Result CSV path (header: match_id,avg_score)
        """
    ...
    def get_matchids(self, full_matchids: list[str]) -> list[str]:
        """Strip region prefix (e.g., 'JP1-7650415714' → '7650415714') and sort."""
        prefix_dash = f"{self.region}-"
        nums = [mid.replace(prefix_dash, "") for mid in full_matchids if mid.startswith(prefix_dash)]
        nums.sort()
        return nums

    def get_matchids2(self, full_matchids: list[str]) -> list[str]:
        """Strip region prefix (e.g., 'JP1_7650415714' → '7650415714') and sort."""
        nums = [mid.replace(self.region_prefix, "") for mid in full_matchids if mid.startswith(self.region_prefix)]
        nums.sort()
        return nums

    def _get_match_timeline_data(self, match_id_num: str, max_retries: int = 5) -> dict:
        """Call Riot Timeline API (with simple rate-limit handling)"""
        url = (f"https://{self.routing}.api.riotgames.com/lol/match/v5/matches/"
               f"{self.region_prefix}{match_id_num}/timeline")
        ...
        for _ in range(max_retries):
            try:
                ...
                time.sleep(2)
        raise ValueError(f"[{match_id_num}] timeline call retry exceeded")
