...
class DataGenerator(BaseRiotClient):

    '''
        api_key : riot api key
        count : match per each player
    '''
    def __init__(self, api_key: str, count: int = 100, region: str = "JP1", **kwargs) -> None:
    ...
    '''
    def get_puuids(self, queue: str, tier: str, division: str) -> List[str]:
        page = 1             #ページ初期値
        summoners_puuids = []       #サモナー名簿
        while True:
            datas = self._request_with_retry(
                f'{self._config.platform_url}/lol/league-exp/v4/entries/{queue}/{tier}/{division}?page={page}'
            )
    ...
    #SUMMONERID -> PUUID
    def get_puuid(self, summonerId: str) -> Optional[str]:
        try:
            data = self._request_with_retry(
                f'{self._config.platform_url}/lol/summoner/v4/summoners/{summonerId}'
            )
            if data is None:
                return None
            return data["puuid"]
    ...
    def is_in_recent_patch(self, game_creation_millisec: float, patch_start_datetime: str) -> bool:
    ...
    def get_matchIds(
        ...
        ts = convert_humantime_to_unixtimestamp(patch_start_datetime)
        ts_7day = convert_humantime_to_unixtimestamp(patch_start_datetime_add_7days)

        matchIdsOver15 = []
        matchIds_response = self._request_with_retry(
            f'{self._config.regional_url}/lol/match/v5/matches/by-puuid/{puuid}/ids?startTime={ts}&queue=420&type=ranked&start=0&count={self.count}'
        )
        matchIds_7day_after_response = self._request_with_retry(
            f'{self._config.regional_url}/lol/match/v5/matches/by-puuid/{puuid}/ids?startTime={ts_7day}&queue=420&type=ranked&start=0&count={self.count}'
        )
        ...
        for matchId in matchIds:
            try:
                match_data = self._request_with_retry(
    ...
    def get_tier_matchIds(
        ...
        min_game_duration: int,
        max_game_duration: int,
        max_workers: Optional[int] = None,  # 後方互換性のため残す（シリアル処理では無視）
    ) -> List[str]:
        """
